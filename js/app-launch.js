...
function renderFacilities(){
 ...
 let list=facilities.filter(f=>f.beach===selected&&[...activeFilters].every(x=>f.filters.includes(x)));
 ...
 $("#facilityMap").innerHTML=list.map((f,i)=>`<button class="marker safe facility-marker" style="left:${f.x}%;top:${f.y}%" aria-label="${f.name} 위치"><span>${i+1}</span></button>`).join("")+(!list.length?'<p class="map-empty">선택한 조건의 시설 위치가 없습니다.</p>':'');
...
const launchNotice=$("#launchNotice"),launchStart=$("#launchStart");
if(sessionStorage.getItem("badamoa-launch-notice")==="seen"){
 launchNotice.classList.add("closed");
}else{
 document.body.classList.add("launch-open");
 requestAnimationFrame(()=>launchStart.focus());
}
launchStart.onclick=()=>{
 sessionStorage.setItem("badamoa-launch-notice","seen");
 launchNotice.classList.add("closed");
 document.body.classList.remove("launch-open");
 setTimeout(()=>launchNotice.setAttribute("aria-hidden","true"),320);
};
